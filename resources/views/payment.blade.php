            ...
            <div class="row">
                <div class="col-xl-8 col-lg-8">
                    ...
                    <ul class="wsus__booking_payment d-flex flex-wrap">
                        ...
                        @if ($paypal->status == 1)
                            <li>
                                <a href="{{ route('pay-with-paypal', $service->slug) }}">
                        ...
                        @if ($mollie->mollie_status ==1)
                            <li>
                                <a href="{{ route('pay-with-mollie',$service->slug) }}">
                        ...
                        @if ($instamojoPayment->status == 1)
                            <li>
                                <a href="{{ route('pay-with-instamojo', $service->slug) }}">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form role="form" action="{{ route('pay-with-stripe', $service->slug) }}" method="POST" class="require-validation" data-cc-on-file="false" data-stripe-publishable-key="{{ $stripe->stripe_key }}" id="payment-form">
                        @csrf
                        <div class="row">
                            <div class="col-xl-12">
                    ...
                    FlutterwaveCheckout({
                        ...
                        callback: function (data) {
                            ...
                            $.ajax({
                                type: 'post',
                                data : {tnx_id,_token},
                                url: "{{ url('pay-with-flutterwave') }}" + "/" + "{{ $service->slug }}",
                                success: function (response) {
            ...
            if(response.is_available){
                var handler = PaystackPop.setup({
                    ...
                    $.ajax({
                        ...
                        url: "{{ url('pay-with-paystack') }}" + "/" + "{{ $service->slug }}",
                        success: function(response) {
                            if(response.status == 'success'){
                                toastr.success(response.message);
                                window.location.href = "{{ route('dashboard') }}";
